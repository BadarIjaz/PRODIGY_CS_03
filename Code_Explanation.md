...
Assumption: We assume a hacker is using a powerful GPU rig capable of trying 10 billion passwords per second. Calculation: Total Combinations / Speed = Time in Seconds. Output: Converts the raw seconds into "Minutes", "Days", or "Centuries" to give the user realistic feedback.
...
COMMON_PASSWORDS List: A predefined list of the most hacked passwords. Logic: If the user's input matches a word in this list, we immediately flag it as "Weak" and skip the rest of the calculations. This prevents passwords like "Password123!" from getting a high score just because they contain numbers and symbols.
...
<KeyRelease>: This is a specific Tkinter event.
It fires the moment the user lifts their finger off a key.
